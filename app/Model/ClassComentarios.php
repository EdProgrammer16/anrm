...
class ClassUsuarios extends ClassConexao {
    ...
    // ========================================================
    protected function signinUsuario($uBI, $uPass, $duracao) {
        $BFetch = $this->DB = $this->conexaoDB()->prepare("SELECT * FROM `usuarios` WHERE usuario_bi = :bi LIMIT 1");
        $this->DB->bindParam(":bi", $uBI, \PDO::PARAM_STR);
        $this->DB->execute();
        $i = 0;
        $Array = [];
        while($Fetch = $BFetch->fetch(\PDO::FETCH_ASSOC)){
            $Array[$i] = [
                'usuario_id'     =>  $Fetch['usuario_id'     ],
                'usuario_nome'   =>  $Fetch['usuario_nome'   ],
                'usuario_nome'   =>  $Fetch['usuario_sobrenome'],
                'usuario_bi'     =>  $Fetch['usuario_bi'     ],
                'usuario_dataCri'=>  $Fetch['usuario_dataCri'],
                'usuario_dataUpt'=>  $Fetch['usuario_dataUpd'],
        ...
        if(count($Array) > 0):
            ...
            if(password_verify($uPass, $Array[0]['usuario_pass'])){
                ...
                $result = $token->createToken([
                    ...
                    'token_LoginUser'=> $Array[0]['usuario_bi'],
    ...
    protected function listarUsuarioID() {
        ...
        $BFetch = $this->DB = $this->conexaoDB()->prepare("SELECT `usuario_id`, `usuario_nome`,  `usuario_sobrenome`, `usuario_bi`, `usuario_dataCri`, `usuario_dataUpd`, `usuario_perm` FROM `usuarios` WHERE `usuario_id` = :usuario_id LIMIT 1");
        $this->DB->bindParam(":usuario_id", $usuarioID, \PDO::PARAM_INT);
        $BFetch->execute();
        $i = 0;
        while($Fetch = $BFetch->fetch(\PDO::FETCH_ASSOC)){
            $Array[$i] = [
                'usuario_id'      =>  $Fetch['usuario_id'     ],
                'usuario_nome'    =>  $Fetch['usuario_nome'   ],
                'usuario_sobrenome'    =>  $Fetch['usuario_sobrenome'],
                'usuario_bi'      =>  $Fetch['usuario_bi'     ],
                'usuario_perm'    =>  $Fetch['usuario_perm'   ],
                'usuario_dataCri' =>  $Fetch['usuario_dataCri'],
                'usuario_dataUpd' =>  $Fetch['usuario_dataUpd']
            ];
            $i++;
        }
        return $Array;
